...
下载 `zy-cinematic-realism-v1.1.2.zip`。压缩包顶层直接包含完整的 `zy-cinematic-realism/` 文件夹，可以安装到支持 Agent Skills 的环境中。

> v1.1 加入导演镜头库。指定导演后默认使用“强烈”模式，通过导演姓名、代表作品和场景专属影像语法，让不同导演真正重新决定这一镜在看什么。

> v1.1.2 强化导演调用行为：只要指定已支持导演，Skill 就会强制按“强烈”执行，并在 Final Prompt 中保留导演英文名、代表作品和场景专属的标志性影像语法。
...
只要指定已支持导演，Skill 就默认并强制按“强烈”执行。Final Prompt 会保留导演的标准英文姓名、两至三部代表作品和对应的标志性影像风格，并将其具体落实到当前场景的故事瞬间、视觉中心、人物调度、机位、镜头、运动、光线、色彩、景深和质感。

| 导演（默认强烈） | 你可能看过的作品 | Final Prompt 中会强化的风格 |
| -- | -- | -- |
...
请让导演姓名、代表作品和标志性影像语法明显主导故事瞬间、人物动作、机位、光线、色彩、景深和质感。
```

> 指定已支持导演时，所有强度词都统一按“强烈”执行；导演姓名、代表作品和场景专属的标志性影像语法必须一同出现在 Final Prompt 中。
...
这组测试固定了同一个故事、人物、年代、地点和证据线索，只改变导演参考。无导演版本保持基础电影现实主义；指定已支持导演后，Skill 强制使用“强烈”模式，在 Final Prompt 中保留导演英文姓名、两至三部代表作品和场景专属的标志性影像语法，并重新组织故事瞬间、视觉中心、摄影机、人物与空间、光线、调色、景深和捕捉质感。
...
4. 上传 Release 中的 `zy-cinematic-realism-v1.1.2.zip`。
...
```text
zy-cinematic-realism-v1.1.2.zip
